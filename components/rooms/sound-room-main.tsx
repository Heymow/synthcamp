...
export function SoundRoomMain() {
  return (
    <article className="glass-panel group relative cursor-pointer overflow-hidden rounded-[2rem] border-white/10 p-1 transition-transform duration-500 active:scale-[0.99] md:rounded-[2.5rem]">
      ...
      <div className="relative z-10 flex flex-col gap-4 p-4 text-left md:gap-5 md:p-5">
        {/* Top strip: ON AIR label + Timer chip */}
        <div className="flex items-center justify-between gap-3 text-white/70">
          ...
          <StatusTimer baseTime={MAIN_ROOM_START} inline />
        </div>

        {/* Main content: avatar + textual info (stacks to row on md+) */}
