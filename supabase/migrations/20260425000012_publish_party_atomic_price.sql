-- SynthCamp — make publish-with-party fully atomic.
--
-- Previously `validate_and_create_listening_party` flipped
-- `releases.status = 'scheduled'` and set `release_date`, returned, and the
-- Next.js publish route then issued a second UPDATE to write
-- `price_minimum`. A connection drop or RLS blip between the two left the
-- release scheduled with a stale (draft-era) price_minimum.
--
-- Add a `p_price_minimum numeric` parameter and write it inside the same
-- UPDATE that flips status. The route computes the price from track count
-- and passes it through.
--
-- Old 3-arg signature is dropped explicitly so PostgREST resolves the new
-- 4-arg version unambiguously.

DROP FUNCTION IF EXISTS public.validate_and_create_listening_party(uuid, uuid, timestamptz);

CREATE OR REPLACE FUNCTION public.validate_and_create_listening_party(
  p_release_id uuid,
  p_room_id uuid,
  p_scheduled_at timestamptz,
  p_price_minimum numeric
...
BEGIN
  ...
  IF p_price_minimum IS NULL OR p_price_minimum < 0 THEN
    RAISE EXCEPTION 'price_minimum must be a non-negative number';
  END IF;
  ...
  -- Single UPDATE: status, release_date, and price_minimum flip together.
  -- Any failure here rolls back the party INSERT (same transaction).
  UPDATE public.releases
  SET status = 'scheduled',
      release_date = p_scheduled_at,
      price_minimum = p_price_minimum
...
GRANT EXECUTE ON FUNCTION public.validate_and_create_listening_party(uuid, uuid, timestamptz, numeric) TO authenticated;
